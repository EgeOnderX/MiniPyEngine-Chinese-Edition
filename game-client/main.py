...
pygame.display.set_caption("MiniPyEngine")

font = pygame.font.Font("fonts/simhei.ttf", 48)
...
button_height = 50
button_x = 100
# Butonlar Çince tanımı ve ekran metni
buttons = {
    "启动服务器": pygame.Rect(button_x, 180, button_width, button_height),  # Start Server
    "开始游戏": pygame.Rect(button_x, 250, button_width, button_height),    # Start Game
    "设置": pygame.Rect(button_x, 320, button_width, button_height),        # Settings
    "关于": pygame.Rect(button_x, 390, button_width, button_height),        # About
    "退出": pygame.Rect(button_x, 460, button_width, button_height)         # Exit
}
...
def draw_settings():
    ...
    sens_label = small_font.render(f"灵敏度: {sensitivity} (click +/-)", True, WHITE)
    screen.blit(sens_label, sens_rect.move(10, 8))

    pygame.draw.rect(screen, GRAY, back_rect, border_radius=12)
    back_label = small_font.render("返回", True, WHITE)
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if settings_active:
                ...
                if buttons["启动服务器"].collidepoint(pos):
                    try:
                        pygame.mixer.Sound("sounds/buttonmenu.mp3").play()
                    except:
                        pass
                ...
                elif buttons["开始游戏"].collidepoint(pos):
                    run_game()
                    try:
                        pygame.mixer.Sound("sounds/buttonmenu.mp3").play()
                    except:
                        pass
                elif buttons["设置"].collidepoint(pos):
                    settings_active = True
                    try:
                        pygame.mixer.Sound("sounds/buttonmenu.mp3").play()
                    except:
                        pass
                elif buttons["关于"].collidepoint(pos):
                    try:
                        pygame.mixer.Sound("sounds/buttonmenu.mp3").play()
                    except:
                        pass
                ...
                elif buttons["退出"].collidepoint(pos):
                    try:
                        for p in psutil.process_iter(['pid', 'name', 'cmdline']):
                            cmdline = p.info.get('cmdline')
